...
def inference_test(device, hidden_dim, num_action, node_feature_size, edge_feature_size, global_dim, batch_size, lr, num_epoch, data_dir, show_result, infer_num=None, check_each = False):
    ...
    saved_model = ActionModel_test(device, hidden_dim, num_action, node_feature_size, edge_feature_size, global_dim)
    saved_model.load_state_dict(torch.load(saved_path))
    saved_model.to(device)
