...
class PosAugmentation(ReadDataset):
    def __init__(self):
        super(ReadDataset,self).__init__()
        self.action_encoder = {'pick':[1, 0, 0], 'place':[0, 1, 0], 'pour':[0, 0, 1]}

    def task_selection(self):
        if self.task == "stacking_5":
            self.stacking_5_pa()
        elif self.task == "mixing_5":
            pass
    
    def stacking_5_pa(self):
        nf = self.node_feature()
        ei_0, ea_0 = self.edge_features(order='1_2_3_4_5', i=0)
        print(ei_0.shape)
        print(ea_0.shape)
...
####Test####
stack_test = PosAugmentation(task = "stacking_5", sort_edge=False)
stack_test.task_selection()