...
class CollectGraph():
    ...
    def collect_stackingv2(self, data_type):
        ...
        stack_action = ['pick','place','pick','place','pick','place','pick','place']
        order_seq_dict = {'1_2_3_45':["Box4", "Box5", "Box3", "Box4 + Box5", "Box2", "Box3", "Box1", "Box2"],
        ...
        if data_type == 'action':
            for order, stack_target_object in order_seq_dict.items():
                goal_state_num = [8, 6, 4, 2]
                for i_g in goal_state_num:
                    goal_node_feature, node_id_to_idx = stack_v2.node_features(order=order, i=i_g)
                    goal_edge_index, goal_edge_attr = stack_v2.edge_features(order=order, i=i_g)

                    for i in range(i_g):
                        x, _ = stack_v2.node_features(order=order, i=i)
                        state_edge_index, state_edge_attr = stack_v2.edge_features(order=order, i=i)

                        cat_edge_index, cat_edge_attr = self.concat_state_and_goal(state_edge_index, state_edge_attr, goal_edge_index, goal_edge_attr)
                        action_code = torch.Tensor(self.action_encoder[stack_action[i]])

                        target_object_index = node_id_to_idx[stack_target_object[i]]
                        target_object_score = np.zeros(stack_v2.max_node_num, dtype=int)
                        target_object_score[target_object_index] = 1
                        target_object_score = torch.from_numpy(target_object_score).type(torch.FloatTensor)
                        graph_dict_data = {'input':{},
                                        'target':{'action':[],
                        ...
                        graph_dict_data['info']['order'] = order
                        graph_dict_data['info']['step'] = i
                        graph_dict_data['info']['demo'] = "stacking_v2"

                        stackingv2_graph.append(graph_dict_data)
            return stackingv2_graph
            ...
            for order, stack_target_object in order_seq_dict.items():
                for i in range(8):
                    ...
                    goal_node_feature, node_id_to_idx = stack_v2.node_features(order=order, i=i+1)
                    goal_edge_index, goal_edge_attr = stack_v2.edge_features(order=order, i=i+1)
    ...
    def collect_stackingv3(self, data_type):
        stackingv3_graph = []
        stack_v3 = ReadDataset(task='stacking_v3', sort_edge=False)

        stack_action = ['pick','place','pick','place','pick','place','pick','place']
        order_seq_dict = {'1_2_345':["Box4", "Box5", "Box3", "Box4 + Box5", "Box2", "Box3 + Box4 + Box5", "Box1", "Box2"],
        ...
        if data_type == 'action':
            for order, stack_target_object in order_seq_dict.items():
                goal_state_num = [8, 6, 4, 2]
                for i_g in goal_state_num:
                    goal_node_feature, node_id_to_idx = stack_v3.node_features(order=order, i=i_g)
                    goal_edge_index, goal_edge_attr = stack_v3.edge_features(order=order, i=i_g)

                    for i in range(i_g):
                        x, _ = stack_v3.node_features(order=order, i=i)
                        state_edge_index, state_edge_attr = stack_v3.edge_features(order=order, i=i)

                        cat_edge_index, cat_edge_attr = self.concat_state_and_goal(state_edge_index, state_edge_attr, goal_edge_index, goal_edge_attr)
                        action_code = torch.Tensor(self.action_encoder[stack_action[i]])

                        target_object_index = node_id_to_idx[stack_target_object[i]]
                        target_object_score = np.zeros(stack_v3.max_node_num, dtype=int)
                        target_object_score[target_object_index] = 1
                        target_object_score = torch.from_numpy(target_object_score).type(torch.FloatTensor)
                        graph_dict_data = {'input':{},
                                        'target':{'action':[],
                        ...
                        graph_dict_data['info']['order'] = order
                        graph_dict_data['info']['step'] = i
                        graph_dict_data['info']['demo'] = "stacking_v3"

                        stackingv3_graph.append(graph_dict_data)
            return stackingv3_graph
            ...
            for order, stack_target_object in order_seq_dict.items():
                for i in range(8):
                    ...
                    goal_node_feature, node_id_to_idx = stack_v3.node_features(order=order, i=i+1)
                    goal_edge_index, goal_edge_attr = stack_v3.edge_features(order=order, i=i+1)
    ...
    def collect_stackingv4(self, data_type):
        stackingv4_graph = []
        stack_v4 = ReadDataset(task='stacking_v4', sort_edge=False)

        stack_action = ['pick','place','pick','place','pick','place','pick','place']
        order_seq_dict = {'1_2345':["Box4", "Box5", "Box3", "Box4 + Box5", "Box2", "Box3 + Box4 + Box5", "Box1", "Box2 + Box3 + Box4 + Box5"],
        ...
        if data_type == 'action':
            for order, stack_target_object in order_seq_dict.items():
                goal_state_num = [8, 6, 4, 2]
                for i_g in goal_state_num:
                    goal_node_feature, node_id_to_idx = stack_v4.node_features(order=order, i=i_g)
                    goal_edge_index, goal_edge_attr = stack_v4.edge_features(order=order, i=i_g)

                    for i in range(i_g):
                        x, _ = stack_v4.node_features(order=order, i=i)
                        state_edge_index, state_edge_attr = stack_v4.edge_features(order=order, i=i)

                        cat_edge_index, cat_edge_attr = self.concat_state_and_goal(state_edge_index, state_edge_attr, goal_edge_index, goal_edge_attr)
                        action_code = torch.Tensor(self.action_encoder[stack_action[i]])

                        target_object_index = node_id_to_idx[stack_target_object[i]]
                        target_object_score = np.zeros(stack_v4.max_node_num, dtype=int)
                        target_object_score[target_object_index] = 1
                        target_object_score = torch.from_numpy(target_object_score).type(torch.FloatTensor)
                        graph_dict_data = {'input':{},
                                        'target':{'action':[],
                        ...
                        graph_dict_data['info']['order'] = order
                        graph_dict_data['info']['step'] = i
                        graph_dict_data['info']['demo'] = "stacking_v4"

                        stackingv4_graph.append(graph_dict_data)
                        print(graph_dict_data)
                        input()
            return stackingv4_graph
            ...
            for order, stack_target_object in order_seq_dict.items():
                for i in range(8):
                    ...
                    goal_node_feature, node_id_to_idx = stack_v4.node_features(order=order, i=i+1)
                    goal_edge_index, goal_edge_attr = stack_v4.edge_features(order=order, i=i+1)
