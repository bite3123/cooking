...
def train_action_test2(device, hidden_dim, num_action, node_feature_size, edge_feature_size, global_dim, batch_size, lr, num_epoch, data_dir):
    ...
    loss_ce_action = nn.CrossEntropyLoss().to(device)
    #loss_ce_object = nn.CrossEntropyLoss().to(device)
    loss_bce_object = nn.BCEWithLogitsLoss().to(device)
    for param in model.parameters():
        param.requires_grad = True
    #for param in model_action.parameters():
    #    param.requires_grad = True
    ...
    for epoch in range(num_epoch):
        ...
        for i, data in enumerate(train_loader):
            ...
            target_object_prob = F.one_hot(target_node_scores.type(torch.int64)).to(device)
            #print(obj_label.shape)
        ...
        with torch.no_grad():
            for i, data in enumerate(val_loader):
                ...
                val_target_object_prob = F.one_hot(val_target_node_scores.type(torch.int64)).to(device)

                val_obj_label = torch.argmax(val_target_node_scores, dim=1).to(device)
                val_L_object = loss_bce_object(val_pred_object_prob, val_target_object_prob.type(torch.float))
