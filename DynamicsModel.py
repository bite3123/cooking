...
class DynamicsModel(nn.Module):
    def __init__(self, device, node_feature_size, global_dim, action_dim):
    ...
    def forward(self, input_data, action, target_node):
        ...
        target_node = (target_node==1).nonzero(as_tuple=False).reshape(-1) #target node에서 1인 부분의 index값
...
class GNNEncoder(nn.Module):
    ...
    def forward(self, input_data, dynamics_emb):
        ...
        if dynamics_emb == None :
            dynamics_emb = torch.zeros((batch[-1]+1, self.global_dim))   
