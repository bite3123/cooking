...
def train_dynamics(device, hidden_dim, num_action, node_feature_size, edge_feature_size, global_dim, batch_size, lr, num_epoch, data_dir):
    dynamics_model = DynamicsModel(device, hidden_dim, node_feature_size, global_dim, num_action)
    ...
    train_dataset = DynamicsDataset(os.path.join(data_dir,'dynamics','train'))
    val_dataset = DynamicsDataset(os.path.join(data_dir,'dynamics','val'))
    ...
    for epoch in range(num_epoch):
        ...
        for i, data in enumerate(train_loader):
            ...
            sim_label = torch.ones(state_u.size(0), 1).squeeze().to(device)
        ...
        with torch.no_grad():
            for i, data in enumerate(val_loader):
                val_state, val_goal, val_target, val_info = data
...
def train_dynamics_test(device, hidden_dim, num_action, node_feature_size, edge_feature_size, global_dim, batch_size, lr, num_epoch, data_dir):
    dynamics_model = DynamicsModel(device, hidden_dim, node_feature_size, global_dim, num_action)
    gnn_encoder = GNNEncoder_test(device, hidden_dim, num_action, node_feature_size, edge_feature_size, global_dim)
    ...
    for epoch in range(num_epoch):
        ...
        for i, data in enumerate(train_loader):
            ...
            goal_x = goal['x'].to(device)
            goal_edge_attr = goal['edge_attr'].to(device)
        ...
        with torch.no_grad():
            for i, data in enumerate(val_loader):
                ...
                val_state_x, val_state_edge_attr, val_state_u, val_state_batch = gnn_encoder(val_state, val_dynamics_emb)

                val_goal_x = val_goal['x'].to(device)
